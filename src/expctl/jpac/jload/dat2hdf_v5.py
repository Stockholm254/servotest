...
def makeHDF5(filename):
	...
	f: HDF5 file
	"""
	f = h5.File(filename, 'w') # Create HDF5 file
	f.create_group("info") # Info group
	f.create_group("data") # Data group
	return f

def addDataSet(data_dir, hdf_file, pmethod, arg):
	f = hdf_file
	# Get processed data and data set info
	rdata, pdata = pmethod(*arg) # Get processed data
	seqname, jrange, smvs, lmvs = readMV(GetMVFileName(data_dir)) # Get MV info
	# Write into HDF5 file
	f['/data'].create_dataset(data_dir, data=pdata)
...
def GetMVFileName(data_dir, fmt='txt'):
	MVfname = glob(data_dir+'/MVs/*.txt')[0]
	return MVfname
