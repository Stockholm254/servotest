...
import inspect
import threading
from .ImViewer import createViewer, loadImage, loadImagefromBuffer
from . import ImViewerQT as imQt
...
class GP_camera:
	...
	def InitCamera(self):
		...
		logger.info('Number of cameras detected: ', num_cams)
		if not num_cams:
	...
	def GrabImages(self, imgbuffer,shutter=0, gain=24., number=0, runname="", foldername=""):
		...
		if autostart == 0:
			...
			if self.NumOfImage > 0:
				...
				if min_gap_time < 80:
					...
					try:
						self.imgbuffer=[]
						self.device.GrabImages(self.imgbuffer,shutter=ShutterTime, gain=gain_mv, number=self.NumOfImage, runname=run_name, foldername=folder_name)
...
if __name__ == '__main__':
	...
	viewer = imQt.createViewer()
	# server = CameraServer("COut1", 60614, message=message, viewer=viewer)
	# t = threading.Thread(name='camera server', target=server.main_loop)
	# qt_thread = threading.Thread(name='QT Viewer', target=imQt.startapp)
	server = CameraServer("COut1", 60614, message=message, viewer=viewer)
	t = threading.Thread(name='camera server', target=server.main_loop)
	t.start()
