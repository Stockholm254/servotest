...
def sendsequence (IFfreqA_hz,IFfreqB_hz, timesA_sec, freqsA_hz, timesB_sec,freqsB_hz): #convert freqs and times to FTW/dFTWs, and cycles, and send to RP!
    ...
    freqsA_FTW=list(map(HzToFTW,freqsA_hz))
    freqsB_FTW=list(map(HzToFTW,freqsB_hz))
    #print freqsA_hz
    #print freqsA_FTW
    #================================================#
    ...
    timesA_cyc=list(map(SecToCycles,timesA_sec))
    timesB_cyc=list(map(SecToCycles,timesB_sec))
