...
class RpDDS:
    ...
    def __init__(self, bitfile="SimonLab_DDDS.bit", fclk_Hz=125e6, maxevents=64):
        self.bitfile = bitfile
        try:
            os.system(f'cat /root/{bitfile} > /dev/xdevcfg')
        else:
    ...
    def sendsequence(self, IFfreqA_hz,IFfreqB_hz, timesA_sec, freqsA_hz, timesB_sec,freqsB_hz): #convert freqs and times to FTW/dFTWs, and cycles, and send to RP!
        ...
        dtA_cyc = np.maximum(1, np.round(timesA_cyc[1:] - timesA_cyc[:-1])) #[max(1,int(round(timesA_cyc[i+1]-timesA_cyc[i]))) for i in range(len(timesA_sec)-1)]
        dtB_cyc = np.maximum(1, np.round(timesB_cyc[1:] - timesB_cyc[:-1])) #[max(1,int(round(timesB_cyc[i+1]-timesB_cyc[i]))) for i in range(len(timesB_sec)-1)]
        #prepend the first one!
        ...
        self.write(self.DDSftw_IF_A_OFFSET, np.unit32(IF_A_FTW)) #JSocket.write_msg(sock,DDSftw_IF_A_OFFSET, int(IF_A_FTW)) #these must be sent as unsigned 32 bit numbers
        self.write(self.DDSftw_IF_B_OFFSET, np.unit32(IF_B_FTW))#JSocket.write_msg(sock,DDSftw_IF_B_OFFSET, int(IF_B_FTW)) #these must be sent as unsigned 32 bit numbers
