...
# code = r"C:\ExperimentSoftwares\Control_Suite_X\servers\FPGA_bit_file\PhotonTimers.bit"  # NORMAL)
...
def RunServer(seq, autostart=0):
	...
	chan = seq.getChannelByName("Photon Timer 3")
	thehardwarevalues = chan.GetHardwareValues()
	save_switch_chan = seq.getChannelByName("Photon Timer 3 Save")
...
def Acquire(T, SeqFolderName='', SeqRunName='michael', save=0):
	...
	if save == 1:
		...
		f = open(full_dir+"PT3"+SeqRunName+'.bin', 'wb')
