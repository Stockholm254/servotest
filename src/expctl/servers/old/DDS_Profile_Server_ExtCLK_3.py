...
#implementation notes: if you want a channel to maintain a value, you must TELL IT TO! otherwise it could go back to the default, or at the end of the sequence go to the I/F value, etc...
#don't assume that a channel will stay at the last value that you set it to (though it likely will, as this software is pretty forgiving): for example, once all DDS channels are done, the software
#will return to I/F values!

#code1 = "C:/Users/Simonlab/Programming/Control Suite/dds_bitfiles\dds_v14.bit" #THIS VERSION WORKS AT LEAST FOR LOADING A MOT AND PLAYING WITH IT A BIT!!
#code1 = "C:/Users/Simonlab/Dropbox/SimonLab/dds_stuff/dds_stuff/hardcodedds/dds.bit" #THIS IS AN OLD VERSION
#code1 = "C:/Users/Graham/Desktop/test/crazy6.bit" #THIS IS THE DEVELOPMENT VERSION!
# code1 = "C:/Users/Simonlab/Programming/Control Suite/dds_PDH.bit" 
# code1 = "C:/Users/Simonlab/Programming/Control Suite/servers/FPGA_bit_file/DDS_freq_out_ExtCLK.bit"
code1 = "C:/ExperimentSoftwares/Control_Suite_X/servers/FPGA_bit_file/DDS_freq_out_ExtCLK.bit"
# code1 = r"C:\Users\Simonlab\Programming\dev\DDS_Profiles_Code_ExtCLK - Copy\dds.bit" 

server = Server("DDS_PDH", 60618)
server.message  = '===============================================\n'
server.message += '==      DDS Profile Frequency Out Server     ==\n'
server.message += '==        for FPGA with External Clock       ==\n'
server.message += '==         PDH Locking Frequency Box         ==\n'
server.message += '==============================================='    
...
FPGAsn = '14290008UX' #'13450006RA' #This must match the S/N of the FPGA inside the PDH DDS box. Can find the serial number via the Opal Kelly FrontPanel interface.


def RunServer(seq, dev, loadorrun, autostart=1):
  ...
  if(loadorrun==LOADMODE):
    ...
    print("Waiting for end of sequence to be announced!")
    while(True):
...
if __name__ == '__main__':
  ...
  print('Connecting to PDH FPGA S/N '+FPGAsn+'...'  + ('success' if dev.OpenBySerial(FPGAsn)==0 else 'failure'))
  print('Getting FPGA PLL Eeprom Config...'  + ('success' if dev.GetEepromPLL22150Configuration(pll)==0 else 'failure'))
  print('Setting FPGA PLL Config to Eeprom vals...'  + ('success' if dev.SetPLL22150Configuration(pll)==0 else 'failure'))
  #FPGAclock=float(pll.GetOutputFrequency(0))
  #Configure FPGA with bit code
  print('Loading Bitfile...' + ('success' if dev.ConfigureFPGA(code1)==0 else 'failure'))
  print('FPGA Clock Frequency: ' + str(FPGAclock) + ' MHz (set in server)')
    ...
    while True:
      ...
      if (command == "RUN"):  # Run the sequence (if we've already received it)
        if server.seq == None:
          ...
          RunServer(server.seq,dev,LOADMODE)
          time_taken = RunServer(server.seq, dev,RUNMODE)
          success = time_taken
      ...
      if command == "QUEUE":  # Run the sequence (if we've already received it)
        if server.seq == None:
          ...
          RunServer(server.seq, dev,LOADMODE, autostart=0)
          #time.sleep(0.2)
          send_msg(clientSocket, server.ReplyHeader() + 'Sequence has been queued... Trigger it whenever!')
          time_taken = RunServer(server.seq, dev, RUNMODE, autostart=0)
          success = time_taken
          print('Time Taken: ' + str(time_taken))
          send_msg(clientSocket, server.ReplyHeader() + 'Successfully ran sequence (' + str(time_taken) + ' seconds).')
          break
      ...
      if command == "SEQ": #Load in a sequence
        seq_data = recv_msg(clientSocket)
        server.seq = pickle.loads(seq_data)  # unpack the sequence
        print("TIME START: "  + str(server.seq.TIME_START))
        print("TIME STOP: "  + str(server.seq.TIME_STOP))
        numChannels = 0
