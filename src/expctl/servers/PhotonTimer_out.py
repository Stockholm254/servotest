...
code = DIR_BITFILE/"PhotonTimers.bit" #code = r"C:\ExperimentSoftwares\Control_Suite_X\servers\FPGA_bit_file\PhotonTimers.bit"
FPGAsn = '1452000AQ7'
	...
	def cmd_queue(self):
		if self.seq is None:
			...
			try:
				...
				if acquire_data == 1:
					e = Acquire(CLK_freq, self.seq.foldername, self.seq.runname, save_data)
					logger.debug(f"Acquire returned {e}")
			except:
...
if __name__ == '__main__':
	...
	server = TimerServer("PTIn1", 60623, message=message)
	server.main_loop()