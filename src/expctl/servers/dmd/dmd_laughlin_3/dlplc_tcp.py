...
- modified by Matt Jaffe for python 3 (6-Nov-2020)
"""
...
class LightCrafterTCP:
    ...
    # this is the ip address the LightCrafter3000 likes to use; check IP address in the TI GUI if having problems connecting
    ...
    def connect(self, ip='', port=0):
        ...
        If no ip and port is specified, tcp_ip is used
        ...
        if self.socket is None:
            print(f'Error: could not open socket to {self.tcp_ip}:{self.tcp_port}')
            return False
        return True
    ...
    def cmd_static_image(self, image):
        ...
        image is a string of Windows BMP encoded data.
        """
...
class Packet():
    ...
    def pack(self):
        ...
        p = struct.pack('%c4BH%dsB'%(self.ORDER,self.size),
            self.p_type,
            self.cmd1, self.cmd2,
            self.flags,
            self.size,
            self.data.encode('latin-1'),
            # self.data.encode('utf-8'),# 'replace'),
            self.checksum)
        self.packet = p
        return p
    ...
    def unpack(self, packet):
        ...
        self.cmd1, self.cmd2,
        self.flags,
        self.size) = struct.unpack('%c4BH'%self.ORDER, packet[:6])
        self.data, self.checksum = struct.unpack('%c%dsB'%(self.ORDER, self.size), packet[6:])
