...
class Dso:
    ...
    def connect(self, str):
        ...
        self.write('*IDN?\n')
        _str = self.read()
        # print('r: ', _str)#, type(_str))
        model_name=_str.split(',')[1]
    ...
    def getBlockData(self): #Used to get image data.
        ...
        length=len(inBuffer)
        self.headerlen = 2 + int(inBuffer[0:2].decode('ascii')[1])
        pkg_length = int(inBuffer[2:self.headerlen].decode('ascii')) + self.headerlen + 1 #Block #48000[..8000bytes raw data...]<LF>
    ...
    def readRawDataFile(self,  fileName):
        ...
        if(self.dataType=='csv'):
            ...
            self.points_num=int(len(wave)/2)   #Calculate sample points length.
            self.dataMode='Fast'
        f.close()
