...
from .rpcounter_reset import RpCounter
...
def consumer(queue, lock):
    ...
    while True:
        ...
        item = queue.get()
        try:
            SaveDataDB(client=client, run_id=item.run_id, counter=item.counter, DataCLK=item.data, save=item.save)
        except:
            with lock:
...
class CounterServer(Server):
    ...
    def __init__(self, name, port, message, q):
        ...
        self.fclk_Hz = 125e6
        self.counter = RpCounter(bitfile=bitfile_path, fclk_Hz=self.fclk_Hz, nbins=1000, ncycles=1250, dac_scale=100)
                ...
                if acquire_data == 1:
                    ...
                    if data == -1:
                        ...
                        if save_switch > 0:
                            ...
                            qitem = CounterItem(run_id=self.seq.run_id, counter=self.seq.counter, data=data, save=save)
                            self.q.put(qitem)
            except:
    ...
    def _acquire(self):
        ...
        if self.n_channels==2:
            logger.error("This bitfile only supports one channel!")
...
if __name__ == '__main__':
    ...
    c = Process(target=consumer, args=(queue, lock), daemon=True)
    c.start()
    server = CounterServer("SOut1", 60621, message=message, q=queue)
    server.main_loop()
    queue.join()