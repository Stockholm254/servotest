...
server = Server("AOut1", 60616)
server.message  = '===========================================\n'
server.message += '==       Dummy Analog Output Server      ==\n'
server.message += '==              for PCI 6723             ==\n'
server.message += '==========================================='    
server.sock = None
    ...
    while True:
      ...
      if command == "QUEUE":  # Run the sequence (if we've already received it)
        if server.seq == None:
          ...
          send_msg(server.sock, server.ReplyHeader() + 'Queue() failed. Sequence has not been imported!')
          break
        else:
          print("Prepared to run the server")
          time_taken = RunServer(server.seq, autostart=0)
          success = time_taken
