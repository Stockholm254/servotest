...
def centroid(data):
    ...
    X, Y = np.meshgrid(x, y)
    x_cen = np.round((data * X).sum() / data.sum(),2)
    y_cen = np.round((data * Y).sum() / data.sum(),2)
    return x_cen, y_cen

def iteration_actuator(servos, picam2, scan_xr):
    """
    ...
    data_x_xr = xr.DataArray(
        data=np.array(data_list_x).reshape(scan_xr.shape[:-1]),
        dims=scan_xr.dims[:-1],
        coords=scan_xr.drop_vars('knobs').coords
    )

    data_y_xr = xr.DataArray(
        data=np.array(data_list_y).reshape(scan_xr.shape[:-1]),
        dims=scan_xr.dims[:-1],
        coords=scan_xr.drop_vars('knobs').coords
    )
