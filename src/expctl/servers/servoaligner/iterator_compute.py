...
def scan_xarray_gen(solver, var_str_list=['MOT_pos_1','MOT_pos_2'], range_list=[3e-4, 3e-4], num_list=[10,10], start_value_list=[0,0], params=dict()):
    iter_list=[]
    coords_dict={}
    ...
    angle_calc_multi_partial=partial(angle_calc_multi, solver=solver, var_str_list=var_str_list, params=params)
    pool=multiprocessing.Pool()
    result=list(tqdm(pool.imap(angle_calc_multi_partial, array_products), total=len(array_products)))
    pool.close()
    pool.join()
...
def scan_xarray_gen_knobs(solver, var_str_list=['servo_deg_1','servo_deg_2'], range_list=[50,50], offset_list=[0,0], num_list=[10,10], knob_deg_list=[0,0,0,0,0,0,0,0]):
    coords_dict={}
    servos_dict={'servo_1x':0, 'servo_1y':1, 'servo_2x':2, 'servo_2y':3, 'servo_3x':4, 'servo_3y':5, 'servo_4x':6, 'servo_4y':7}
    iter_list=[[knob_deg_list[i]] for i in range(8)]
    
    for i in range(len(var_str_list)):
        knob_index=servos_dict[var_str_list[i]]
        iter_list[knob_index]=np.linspace(knob_deg_list[knob_index]-range_list[i]+offset_list[i], knob_deg_list[knob_index]+range_list[i]+offset_list[i], num_list[i])
        coords_dict[var_str_list[i]]=np.linspace(knob_deg_list[knob_index]-range_list[i]+offset_list[i], knob_deg_list[knob_index]+range_list[i]+offset_list[i], num_list[i])

    result=list(product(*iter_list))
...
if __name__=="__main__":
    ...
    result_xr=scan_xarray_gen(solver_0, var_str_list=['MOT_pos_rel','Cav_pos_1'], range_list=[2e-2, 2e-4], num_list=[10,10], start_value_list=[MOT_pos_rel, Cav_pos_1])
    current_dir=os.path.dirname(__file__)
    file_path=os.path.join(current_dir, "angles0304_4.nc")
