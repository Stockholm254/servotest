...
#
#ss
N=1#how many data points to collect
normd = 20
offset_type = 'zero' # 'pm' or 'rand' or 'lin' or 'zero'
MASTER = "B"  # A=upper path, B=lower path, divide groups
#
for i in range(N):
    ...
    if offset_type == 'pm':#plus minus
        offset = cord_pm_offset(N,normd,i)
    elif offset_type == 'rand':#randomly bias four axis
        offset = random_norm_offset(N,normd)
    elif offset_type == 'lin':#span 4-d space on 2-d subspace
        if MASTER == "A":
