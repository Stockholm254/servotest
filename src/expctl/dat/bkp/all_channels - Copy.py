...
# Photon counter
PhotonCounter = photon_counter.newChannel(0, "Photon Counter", steady_state_value=0, max_value=1, graph=0, ctype='Slave', master=SPCM_ttl)
# Photon timer
PhotonTimer = photon_timer.newChannel(1, "Photon Timer", steady_state_value=0, max_value=1, graph=0, ctype='Slave', master=SPCM_ttl)
	...
	for jj, _chan in enumerate(_seq.allChannels):
		if _chan != None:
			if _chan.chantype == 'Slave':
				all_copyChans.append([_chan.master, _chan])
