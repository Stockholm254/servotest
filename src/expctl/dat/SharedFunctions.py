...
def Transport(times, acceleration, distance, Lat_Bot_Freq=80, Lat_Top_Freq=80, Max_df=10.):
	wavelength = 784*1e-9 #m
	g = 9.81 #m/s^2
	dfMax = Max_df * Unit.MHz() #maximum velocity = 3.91m/s
	MHz = 1e6
	Lat_Bot_MHz = Lat_Bot_Freq*1e6
	Lat_Top_MHz = Lat_Top_Freq*1e6
	...
	if abs(df) <= dfMax*MHz:
		...
		# Frequency Ramp
		df_MHz = df*1e6
		RP_trans_a.SetInterval(times_acc, Lat_Bot_MHz, Lat_Bot_MHz+df_MHz)
		# DDS_LT.SetInterval(times_acc, Lat_Top_Freq, Lat_Top_Freq-df)
		RP_trans_a.SetInterval(times_deacc, Lat_Bot_MHz+df_MHz, Lat_Bot_MHz)
		...
		RP_trans_a.SetInterval(times_acc, Lat_Bot_MHz, Lat_Bot_MHz+df_MHz)
		# DDS_LT.SetInterval(times_acc, Lat_Top_Freq, Lat_Top_Freq-df)
		RP_trans_a.SetInterval(times_cv, Lat_Bot_MHz+df_MHz)
