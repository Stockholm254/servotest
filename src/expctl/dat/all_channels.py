...
all_sequences = ([
  ...
  Sequence("DMD sequence",              host='127.0.0.1' , port=PORT_DMD,       max_channels=17, graph=0), #IP_RYDFLURRY
  Sequence("SmarAct sequence",          host=IP_RYDFLURRY, port=PORT_SMARACT,   max_channels=3 , graph=0),
  Sequence("rp DOG 1",          host='127.0.0.1' ,   port=PORT_DIGITAL,   max_channels=15, graph=0, seq_type="MASTER"), #, seq_type="MASTER"
  Sequence("Servoaligner",          host=IP_ALIGNER ,   port=PORT_ALIGN,   max_channels=4, graph=0)
  ])

digital_seq1, analog_seq1, cam, cam2, dds_1, dds_pdh, dds_2, photon_timer, photon_timer_2, photon_timer_3, adc_1, photon_counter, lb_1, lb_2, lb_3, lb_4, ad_1, rfsoc_1, rfsoc_2, rp_ddds_1, kinesis_2, kinesis_4, atten_1, dmd, smaract, rpdog_1, servoaligner = all_sequences # WE DO IT IN THIS ORDER SO THAT ONE CANNOT GET AWAY WITH CREATING A NAMED SEQUENCE WHICH IS NOT IN THE ARRAY OF ALL SEQUENCES!!
...
EF3            = analog_seq1.newChannel(23, "EFilter 3",       system='EField',  steady_state_value=0.0,   max_value=60.0, graph=0, transform_v=tran.ElectrodeGain3)
...
servo_1x = servoaligner.newChannel(0, "Servo 1x angle", system='VLAT', steady_state_value=355.5, max_value=720.0, graph=0)
servo_2x = servoaligner.newChannel(1, "Servo 2x angle", system='VLAT', steady_state_value=355.5, max_value=720.0, graph=0)
servo_1y = servoaligner.newChannel(2, "Servo 1y angle", system='VLAT', steady_state_value=355.5, max_value=720.0, graph=0)
servo_2y = servoaligner.newChannel(3, "Servo 2y angle", system='VLAT', steady_state_value=355.5, max_value=720.0, graph=0)
