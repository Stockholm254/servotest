...
PGC_first = 0
# Tab:Transport
MV(Trans_dist_mm, min=-50.0, max=100.0, init=32.0, inc=5e-2, digits=2)
Trans_acc_g = 100.0
Trans_RoundTrip = 0.0
Trans_MaxF_MHz = 5.0
Trans_Blue_ttl = 0.0
...
MW_CW_ttl = 1
MWaves_AD_ttl = 1
MV(MWaves_Freq_MHz, min=35, max=4400, init=1500, inc=0.1, digits=1)
MV(MWaves_pwr_dBm, min=-4, max=5, init=5, inc=0.5, digits=1) 
MWpulse1_ttl = 0
MV(MWpulse1_start_us, min=-10000, max=10000, init=0, inc=1, digits=1)
MV(MWpulse1_dur_us, min=0, max=10000, init=0, inc=1, digits=3) 
MWpulse2_ttl = 0
MV(MWpulse2_start_us, min=-10000, max=10000, init=0, inc=1, digits=1)
MV(MWpulse2_dur_us, min=0, max=10000, init=0, inc=1, digits=3) 
# Tab:REP_Durations
...
#STP_PRB  =     [(0., PRB_gap_ttl, t_gap2, PRB_gap_ttl), (t_gap2, PRB_ttl, t_PRB-0.5*PRB_time_ms*Unit.ms(), PRB_ttl), (t_PRB-0.5*PRB_time_ms*Unit.ms(), 0., t_PRB, 0.)]
on_time = PRB_time_ms*Unit.ms()/PRB_subrep*PRB_subrep_dutyc
off_time = PRB_time_ms*Unit.ms()/PRB_subrep*(1.-PRB_subrep_dutyc)
subcy_time = on_time + off_time
STP_PRB_EOM = [(0., PRB_gap_ttl, t_gap2, PRB_gap_ttl), ]
for i in range(int(PRB_subrep)):

	STP_PRB_EOM.append((t_gap2+subcy_time*i, PRB_ttl, t_gap2+subcy_time*i+on_time, PRB_ttl))
	STP_PRB_EOM.append((t_gap2+subcy_time*i+on_time, 0., t_gap2+subcy_time*(i+1), 0.))
...
STP_MW = [(0., 0, t_gap2, 0), ]
for i in range(int(PRB_subrep)):
	STP_MW.append((t_gap2+subcy_time*i+on_time+MWpulse1_start_us, MWpulse1_ttl, t_gap2+subcy_time*i+on_time+MWpulse1_start_us+MWpulse1_dur_us, MWpulse1_ttl))
	STP_MW.append((t_gap2+subcy_time*i+on_time+MWpulse1_start_us+MWpulse1_dur_us, 0, t_gap2+subcy_time*(i+1), 0.))
	#pulse 2
	STP_MW.append((t_gap2+subcy_time*i+on_time+MWpulse2_start_us, MWpulse2_ttl, t_gap2+subcy_time*i+on_time+MWpulse2_start_us+MWpulse2_dur_us, MWpulse2_ttl))
	STP_MW.append((t_gap2+subcy_time*i+on_time+MWpulse2_start_us+MWpulse2_dur_us, 0, t_gap2+subcy_time*(i+1), 0.))
...
LB3_freq.SetInterval(times_Init, Floq_SB2_Freq_MHz)
LB3_pow.SetInterval(times_Init, Floq_SB2_pwr_dBm)
LB3_ttl.SetInterval(times_Init, 1)
# ADF435X
...
BiasZ.SetInterval(times_transport.afterStart(times_transport.length()/2.0), PGC_BiasZ_G, dRSC_Bz_G)
 ...
 #  	REP0_ttl.SetInterval(times_transport.afterward(0), 0)
 	# MOTCoil.SetInterval(times_transport.afterStart(Trans_PGC_delay_ms*Unit.ms()).afterward(0), MOT_CoilCurr)
...
LB3_freq.SetSteadyStateValue(Floq_SB2_Freq_MHz)
LB3_pow.SetSteadyStateValue(Floq_SB2_pwr_dBm)
LB3_ttl.SetSteadyStateValue(1)
