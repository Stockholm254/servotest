...
Trans_GDEP_ttl = 1
Trans_GDEP_pwr = 5
MV(ParamHeat_freq_kHz, min=10, max=1000.0, init=110, inc=0.1, digits=2)
# Tab:dRSC
dRSC_switch   = 1
dRSC1_repetitions = 1
dRSC_Dur_ms   = 5
...
PRB_L_df_MHz = 10.0
MV(t_subrep_us, min=0, max=30000, init=50, inc=1, digits=3)
MV(Prb_P_wait_us, min=0, max=30000, init=0, inc=1, digits=3)
MV(Prb_P_dur_us, min=0, max=30000, init=25, inc=1, digits=3)
...
times_dRSC2 = []
times_Wait1p25 = times.append(Wait1p25_ms*Unit.ms(), "Wait In Cav AFTER dRSC")
...
times_Wait1p5 = times.append(Wait1p5_ms*Unit.ms(), "Wait In Cav AFTER dRSC + ELAT ramp")
...
# ramp cavity ODT on, VLAT should be still on there at 'c1' values
if RampCODT_switch == 1:
	...
	#Sacher2_ttl.SetInterval(times_ramp_odt.afterStart(0), Ramp1_ELAT_TTL)
	...
	#Sacher2_pwr.SetLogRamp(times_ramp_odt, Sacher2_pwr.GetLastValue(), Ramp1_ELAT_pwr, sample_rate=samprate)
...
if PRB_mode == 1: # Static cloud in the cavity
	...
	ODT2_ttl.SetInterval(times_Prb.beforeStart(100*Unit.us()), PRB_CODT_ttl)
...
RFSOC1_6.SetSteadyStateValue(ParamHeat_freq_kHz*Unit.kHz())
