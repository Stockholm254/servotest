...
from expctl.dat.all_channels import UV_pwr
...
MV(DMD_Defocus, min=-10000, max=10000, init=780, inc=10, digits=1)
...
MV(SmarAct_x_V, min=0, max=100, init=0, inc=0.1, digits=1)
MV(SmarAct_y_V, min=0, max=100, init=0, inc=0.1, digits=1)
MV(SmarAct_z_V, min=0, max=100, init=0, inc=0.1, digits=1)
DMD_mode = 0
MV(DMD_Prb_X, min=-300, max=300, init=0, inc=0.1, digits=1)
MV(DMD_Prb_Y, min=-300, max=300, init=0, inc=0.1, digits=1)
MV(DMD_Prb_Waist, min=0, max=300.0, init=12.0, inc=0.01, digits=2)
MV(DMD_Prb_Phase, min=0, max=2*pi, init=0.0, inc=0.01, digits=2)
DMD_Prb_UsePhase = 0
DMD_Prb_UseAmp = 0
# Tab:E_Filter
# MV(Ex, min=-20., max=20., init=-0.0, inc=2.e-1, digits=4)
# MV(Ey, min=-20., max=20., init=0.0, inc=2.e-1, digits=4)
MV(E_par, min=-20., max=20., init=1.35, inc=2.e-1, digits=4)
MV(E_perp_xy, min=-20., max=20., init=8.4, inc=2.e-1, digits=4)
MV(Ez, min=-20., max=20., init=0.0, inc=2.e-1, digits=4)
...
MV(ExTrim, min=-20., max=20., init=0, inc=2.5e-3, digits=4)
MV(EyTrim, min=-20., max=20., init=0, inc=2.5e-3, digits=4)
MV(EzTrim, min=-20., max=20., init=0, inc=2.5e-3, digits=4)
...
CLIP_el_switch = 0 # clip the electrode voltages for easier scans
ElMod_switch = 0
ElMod_Volt = 1.0
ElMod_freq_kHz = 1.0
# Tab:MOT
...
MV(B_1_X_G, min=-3.0, max=3.0, init=0.0, inc=1e-3, digits=4)
MV(B_1_Y_G, min=-3.0, max=3.0, init=0.0, inc=1e-3, digits=4)
MV(B_1_Z_G, min=-3.0, max=3.0, init=0.0, inc=1e-3, digits=4)
...
OP_780_pwr = 3.3
MV(WP_Lambda2_deg, min=-719.999, max=719.999, init=355.5, inc=0.05, digits=3)
MV(WP_Lambda4_deg, min=-719.999, max=719.999, init=351.3, inc=0.05, digits=3)
# Tab:REP_Durations
...
MV(PRB_CtrlAOM_df_MHz, min=0, max=50, init=0.00, inc=0.1, digits=2)
Blue_ON_ms = 0.0
# Tab:Subrep
PRB_pwr_low = 3.3
PRB_pwr_high = 3.6
...
# Tab:Following
FOLLOW_mode = 0
FOLLOW_LAT_SWITCH = 0.0
FOLLOW_GDEP_ms = 1
FOLLOW_GDEP_pwr = 4.8
FOLLOW_PRB_ms = 10.0
FOLLOW_PRB_pwr = 3.3
FOLLOW_PRB_df_MHz = 10.0

# Tab:Autoalign
MV(servo_1x_deg, min=-719.999, max=719.999, init=355.5, inc=0.05, digits=3)
MV(servo_2x_deg, min=-719.999, max=719.999, init=355.5, inc=0.05, digits=3)
MV(servo_1y_deg, min=-719.999, max=719.999, init=355.5, inc=0.05, digits=3)
MV(servo_2y_deg, min=-719.999, max=719.999, init=355.5, inc=0.05, digits=3)
...
Bx_1_G = B_1_G*np.sin(np.deg2rad(B_1_theta_deg))*np.cos(np.deg2rad(B_1_phi_deg)) + B_1_X_G
By_1_G = B_1_G*np.sin(np.deg2rad(B_1_theta_deg))*np.sin(np.deg2rad(B_1_phi_deg)) + B_1_Y_G
Bz_1_G = B_1_G*np.cos(np.deg2rad(B_1_theta_deg)) + B_1_Z_G
...
if BRamp2_ttl == 1:
    ...
    times_BRamp2_settle = times.append(1.0*Unit.ms(),'B Ramp 2 Settle')
...
if FOLLOW_mode == 1:
    times_follow_gdep = times.append(FOLLOW_GDEP_ms*Unit.ms(), 'GDEP after probe')
    times_follow_prb = times.append(FOLLOW_PRB_ms*Unit.ms(), 'follow CavPrb')
    times_Prb_n_Follow = times_Prb.afterStart(times_Prb.length()+FOLLOW_GDEP_ms*Unit.ms()+FOLLOW_PRB_ms*Unit.ms())
    #times_Prb_n_Follow = times_Prb.afterward(FOLLOW_GDEP_ms*Unit.ms()+FOLLOW_PRB_ms*Unit.ms())
elif PRB_mode >0:
    times_Prb_n_Follow = times_Prb
    pass
else:
    pass
...
UV_ttl.SetInterval(times_Init, UV_seq_ttl)
...
rp_trig.SetInterval(times_Init.afterStart(17*Unit.ms()), 1)
rp_trig.SetInterval(times_Init.afterStart(17*Unit.ms()).afterward(0), 0)
...
Blue_pwr.SetInterval(times_Init, 0)
ODT2_pwr.SetInterval(times_Init, 0)
Nufern0_pwr.SetInterval(times_Init, 0) #PRB_pwr_low
CavPrbEOM_pwr.SetInterval(times_Init, PRB_pwr_high)
VImg_pwr.SetInterval(times_Init, 0)
...
UV_pwr.SetInterval(times_Init, UV_gap_pwr, UV_seq_pwr)
# DDS seq 1
DDS_REP.SetInterval(times_Init, REPFREQ)
DDS_MOT.SetInterval(times_Init, MOTFREQ)
RFSOC1_784Lock.SetInterval(times_Init, PDH960_SciCavOffs_MHz*Unit.MHz())
# DDS seq 2
DDS_CavPrbAOM.SetInterval(times_Init, CavPrb_FreqOffset_MHz*Unit.MHz())
DDS_chan1.SetInterval(times_Init, 80.0+Img_det23_MHz)
DDS_OptPump1.SetInterval(times_Init, 99.0)
DDS2_3.SetInterval(times_Init, 123.0*Unit.MHz())
...
#Autoaligner
servo_1x.SetInterval(times_Init, servo_1x_deg)
servo_2x.SetInterval(times_Init, servo_2x_deg)
servo_1y.SetInterval(times_Init, servo_1y_deg)
servo_2y.SetInterval(times_Init, servo_2y_deg)
...
DMD_switch.SetInterval(times_Init, DMD_mode) # different DMD modes: 0) LG modes 1) abberration scanning beamlets
DMD_prb_x.SetInterval(times_Init, DMD_Prb_X)
DMD_prb_y.SetInterval(times_Init, DMD_Prb_Y)
DMD_prb_waist.SetInterval(times_Init, DMD_Prb_Waist)
DMD_prb_phase.SetInterval(times_Init, DMD_Prb_Phase)
DMD_prb_phasemap.SetInterval(times_Init, DMD_Prb_UsePhase)
DMD_prb_ampmap.SetInterval(times_Init, DMD_Prb_UseAmp)

#SmarAct
SMARACT_vx.SetInterval(times_Init, SmarAct_x_V)
SMARACT_vy.SetInterval(times_Init, SmarAct_y_V)
SMARACT_vz.SetInterval(times_Init, SmarAct_z_V)
...
if dRSC_switch == 1:
    ...
    for this_time in times_dRSC:
        ...
        AUX_ttl.SetInterval(this_time, dRSC_2toFp_ttl) # dRSC
        ...
        D1Laser1_ttl.SetInterval(this_time.afterward(0),0)
        ...
        if i<int(dRSC_repetitions)-1: # Dont compress on the last rep
            ...
            if c1_Pump_ttl:
                D1Laser1_ttl.SetInterval(this_time.afterward(c1_hold_time_ms*Unit.ms()), c1_Pump_ttl)
                D1Laser1_pwr.SetInterval(this_time.afterward(c1_hold_time_ms*Unit.ms()), c1_Pump_pwr)
                # Use proper RF source for driving the EOM
                AUX_ttl.SetInterval(this_time.afterward(c1_hold_time_ms*Unit.ms()), dRSC_2toFp_ttl) # dRSC
                AUX_ttl.SetInterval(this_time.afterward(c1_hold_time_ms*Unit.ms()).afterward(0), 0) # dRSC

                AUX2_ttl.SetInterval(this_time.afterward(c1_hold_time_ms*Unit.ms()), 1) # MUST BE ONE TO ENSURE THAT LAB BRICK IS TRIGGERED ON
                AUX2_ttl.SetInterval(this_time.afterward(c1_hold_time_ms*Unit.ms()), 0)
...
if Ramp1_switch == 1:
    if ELdRSC_switch==0:
        Shut_HLAT_ttl.SetInterval(times_ramp1, 0) #TODO temporary hack!
...
if BRamp1_switch == 1:
    Shut_HLAT_ttl.SetInterval(times_BRamp1, 0)
...
if OP_REP_switch == 1:
    ...
    if OP_REP_RF_type == 0: # typically for dRSC | strong repump (1->2') sideband (6831 MHz from LabBrick 5626 thru big amp) and relatively weak carrier (550 MHz detuned from 2->2') by zeroing BesselJ_0
        ...
        AUX2_ttl.SetInterval(times_OP.beforeStart(500), 1)
...
if OP_780_switch == 1:
    ...
    EITPrbEOM_ttl.SetInterval(times_OP, 1)
...
if PRB_mode == 1: # Static cloud in the cavity
    ...
    # set the UV PRB pwr
    UV_pwr.SetInterval(times_Prb, UV_PRB_pwr)
    UV_pwr.SetInterval(times_Prb.afterward(0), UV_seq_pwr)
    ...
    PRB_pwr_ttl.SetInterval(times_Prb, 1) #prb with high rf power
    PRB_pwr_ttl.SetInterval(times_Prb.afterward(0), 0)
    

    # Blue beam
    # Blue_ttl.SetInterval(times_delay, PRB_Ctrl_ttl)
    Blue_ttl.SetInterval(times_delay, 0)
    ...
    if PRB_sweep_num == 1:
        if N_subreps>1:	
            ...
            # actually don't skip the robe gaps during the frequency sweep but wait so we don't loose any info!
            # t_gap1 = PRB_gaps_ms*Unit.ms()
            # t_PRB_OP = t_gap1 + PRB_OP_REP_total_ms*Unit.ms()
            # t_gap2 = t_PRB_OP + PRB_gaps_ms*Unit.ms()
            # t_PRB = t_gap2 + PRB_time_ms*Unit.ms()
            # t_PRB_total = t_PRB*PRB_repetitions
            _freqs = np.linspace(CavPrb_f0, CavPrb_f1, int(PRB_repetitions)+1, endpoint=True)
            for r in range(int(PRB_repetitions)):
                #RFSOC1_CavPrbEom.SetInterval(times_Prb_latetrig.afterStart(r*t_PRB).afterward(t_gap2), _freqs[r]-1, _freqs[r]+1)
                # RFSOC1_CavPrbEom.SetInterval(times_Prb_latetrig.afterStart(r*t_PRB).afterward(t_gap2), PRB_f0_MHz*Unit.MHz(), PRB_f0_MHz*Unit.MHz())
                RFSOC1_CavPrbEom.SetInterval(times_Prb_latetrig.afterStart(r*t_PRB+t_gap2).afterward(PRB_time_ms*Unit.ms()), _freqs[r], _freqs[r+1])
        ...
        for sn in range(int(PRB_sweep_num)):
            ...
            # RFSOC1_CavPrbEom.SetInterval(times_Prb_latetrig.afterStart(sn*t_PRB+t_gap2).afterward(PRB_time_ms*Unit.ms()), CavPrb_f0, CavPrb_f1)


        if PRB_CtrlAOM_df_MHz > 0:
                BluePrbSweep_f0 = (PRB_CtrlAOM_f0_MHz - PRB_CtrlAOM_df_MHz/2.)*Unit.MHz()
                BluePrbSweep_f1 = (PRB_CtrlAOM_f0_MHz + PRB_CtrlAOM_df_MHz/2.)*Unit.MHz()
                # RFSOC1_4.SetInterval(times_Prb_latetrig, BluePrbSweep_f0, BluePrbSweep_f1)
                for r in range(int(PRB_repetitions)):
                    RFSOC1_4.SetInterval(times_Prb_latetrig.afterStart(r*t_PRB).afterward(t_PRB), BluePrbSweep_f0, BluePrbSweep_f1)
                    # RFSOC1_4.SetInterval(times_Prb_latetrig.afterStart(r*t_PRB+t_gap2).afterward(PRB_time_ms*Unit.ms()), BluePrbSweep_f0, BluePrbSweep_f1)
    ...
    # Trigger the scope
    Scope_trig.SetInterval(times_Prb_n_Follow, 1)
    # Scope_trig.SetInterval(times_Prb.beforeEnd(PRB_time_us*Unit.us()), 1)
    Scope_trig.SetInterval(times_Prb_n_Follow.afterward(0), 0)
    # Trigger the SPCM
...
if Img_switch == 2: # Absorption imaging
    ...
    times_IMG = Imaging(times, 2, Img_prep_time_us*Unit.us(), Img_TOF_ms*Unit.ms(), Img_time_us*Unit.us(), Img_drop_time_ms*Unit.ms(), Img_ABS_pwr, 0., None, None, blue_img_ttl=Blue_img_ttl, blue_img_pwr=PRB_Ctrl_pwr)
    #VImg_pwr.SetInterval(times_IMG, Img_ABS_pwr) # set the imaging power int he function instead so we can control the timing finer
    # Elliptical lattice
...
if Img_switch == 3: # Single shot imaging
    times_IMG = Imaging(times, 3, Img_prep_time_us*Unit.us(), Img_TOF_ms*Unit.ms(), Img_time_us*Unit.us(), 0., 0., 0., 0., 0., blue_img_ttl=Blue_img_ttl, blue_img_pwr=PRB_Ctrl_pwr)
    # Elliptical lattice
...
if Img_switch == 2:
    ...
    # open MOT shutter
    Shut_MOT_ttl.SetInterval(times_IMG_DEP, 1) #.beforeStart(1*Unit.ms())
...
if FOLLOW_mode == 1: # for following the cavity, get bare cavity transmission; tur off lattice and blow away/depump cloud with GDEP
    # Turn off the lattice 
    LAT0_ttl.SetInterval(times_follow_gdep, 0)
    LAT1_ttl.SetInterval(times_follow_gdep, 0)
    LAT2_ttl.SetInterval(times_follow_gdep, 0)
    Sacher2_ttl.SetInterval(times_follow_gdep, 0)

    DEP_det_MHz = Rb.DDS_MOT_22(GDEP_det_MHz)
    MOT2_ttl.SetInterval(times_follow_gdep, 1)
    MOT2_ttl.SetInterval(times_follow_gdep.afterward(0), 0)
    MOT2_pwr.SetInterval(times_follow_gdep, FOLLOW_GDEP_pwr)
    MOT2_pwr.SetInterval(times_follow_gdep.afterward(0), 0)
    if GDEP_switch != 1:
        DDS_MOT.SetInterval(times_follow_gdep.beforeStart(5*Unit.ms()), DDS_MOT.GetLastValue(), DEP_det_MHz) # DOES THIS DURING BRamp1!!!


    # Turn on/off the cavity probe beam
    Nufern0_ttl.SetInterval(times_follow_prb, 1)
    Nufern0_ttl.SetInterval(times_follow_prb.afterward(0), 0)
    Nufern0_pwr.SetInterval(times_follow_prb, FOLLOW_PRB_pwr)
    # Turn on/off the cavity probe EOM
    EITPrbEOM_ttl.SetInterval(times_follow_prb, 1)
    EITPrbEOM_ttl.SetInterval(times_follow_prb.afterward(0), 0)

    # Sweep Probe Freq
    CavPrb_f0 = (PRB_f0_MHz+FOLLOW_PRB_df_MHz)*Unit.MHz()
    CavPrb_f1 = (PRB_f0_MHz-FOLLOW_PRB_df_MHz)*Unit.MHz()
    #RFSOC1_CavPrbEom.SetInterval(times_CavPrb, CavPrb_f0, CavPrb_f1)
    RFSOC1_CavPrbEom.SetInterval(times_follow_prb, CavPrb_f0, CavPrb_f1)
    # Trigger the scope
    SPCM_ttl.SetInterval(times_follow_prb, 1)
    SPCM_ttl.SetInterval(times_follow_prb.afterward(0), 0)
...
UV_pwr.SetInterval(times_FinalWait, UV_seq_pwr, UV_gap_pwr)
...
UV_pwr.SetSteadyStateValue(UV_gap_pwr)
# DDS seq 1
...
RFSOC1_784Lock.SetSteadyStateValue(PDH960_SciCavOffs_MHz*Unit.MHz())
# DDS PDH seq
...
DDS_OptPump1.SetSteadyStateValue(99.0*Unit.MHz())
DDS2_3.SetSteadyStateValue(123.0*Unit.MHz())
# Lab Bricks
...
SMARACT_vz.SetSteadyStateValue(SmarAct_z_V)